...
- `Mother.age`: the mother's age (discrete quantitative variable);
...
- `Gestation.weeks`: number of gestation's weeks (discrete quantitative variable);
...
Before going on, let us also assess the presence of **multicollinearity** by computing the variance inflation factors (VIF) of `mod7`:

```{r}
vif(mod7)
```

`Gestation.weeks` and `Length` have very high values of VIF. To address this problem, let us try to exclude one of the two variables, for example `Length`:

```{r}
mod8 <- update(mod7,~.-Length-I(Length^2))
summary(mod8)
```

We have a significant decrease of the adjusted $R^2$, therefore, instead of removing the `Length` variable, we try to include an interaction term between `Length` and `Gestation.weeks`:

```{r}
mod9 <- update(mod7,~.+Length:Gestation.weeks)
summary(mod9)
```

In this case, the adjusted $R^2$ is comparable to that of `mod7`.<br> Since the p-value associated to the quadratic term in `Gestation.weeks` is high, let us try to omit it:

```{r}
mod10 <- update(mod9, ~.-I(Gestation.weeks^2))
summary(mod10)
```

The adjusted $R^2$ has stayed the same. In addition, both the AIC and BIC prefer `mod10` to `mod7`:

```{r}
AIC(mod7,mod8,mod9,mod10)
BIC(mod7,mod8,mod9,mod10)
```

Furthermore, we have solved the problem of multicollinearity, since now the VIFs are the following:

```{r}
vif(mod10, type = "predictor")
```

Therefore, we consider `mod10` as the best model.<br> Let us analyse its residuals:
...
```{r}
mean(residuals(mod10));sd(residuals(mod10))
```
...
The test leads us to reject the null hypothesis, thus we conclude that the residuals are heteroscedastic, meaning that they do not have a costant variance, as can be seen by the "Residuals vs Fitted" plot at the top left of the previous figure. 
...
new_data <- data[-index,]
mod11 <- lm(Weight ~ Pregnancies.number + Gestation.weeks*Length + Cranium + 
                Sex + I(Length^2), data = new_data)
summary(mod11)
```

The removal of the observation causes an increase in the adjusted $R^2$, but also a substantial increase in the p-value of the coefficient associated to `Gestation.weeks:Length`, namely the interaction term. For this reason, we try and look at the model without its term:  

```{r}
mod12 <- update(mod11,~.-Gestation.weeks:Length)
summary(mod12)
AIC(mod10,mod11,mod12)
BIC(mod10,mod11,mod12)
```

Both AIC and BIC lead us to choose `mod12` as the best multiple linear regression model. Let us consider its residuals:
...
```{r}
bptest(mod10);bptest(mod12)
```

The p-value of the Breush-Pagan test is now significantly higher than before, so that we do not reject the hypothesis of homoscedasticity, if we set the significance level to 0.01. Therefore, we can conclude that `mod12` can be used to make a reliable prediction of a newborn's weight.

Let us try to simplify the model in order to visualize the data, nevertheless keeping the essential ingredients. To this aim, we divide the data according to the newborn's sex. Then, we choose other two variables between the predictors of `mod12`, which are `Gestation.weeks`, `Pregnancies.number`, `Length` and `Cranium`. For example, let us consider the couple of variables given by `Gestation.weeks` and `Length`. Therefore, we obtain the following 3-dimensional scatterplot:
